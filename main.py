...
# Estilos CSS ejecutivos para personalizar el acabado visual
st.markdown("""
    <style>
...
# B. Filtros multiselect
weeks_available = sorted(df_filtered["WEEK"].unique().tolist())
...
cities_available = sorted(df_filtered["CIUDAD"].unique().tolist())
selected_cities = st.sidebar.multiselect("Ciudad", options=cities_available)
if selected_cities:
    df_filtered = df_filtered[df_filtered["CIUDAD"].isin(selected_cities)]

zones_available = sorted(df_filtered["ZONA AFECTADA"].unique().tolist())
selected_zones = st.sidebar.multiselect("Zona Afectada", options=zones_available)
if selected_zones:
    df_filtered = df_filtered[df_filtered["ZONA AFECTADA"].isin(selected_zones)]

crit_available = sorted(df_filtered["CRITICIDAD"].unique().tolist())
selected_crit = st.sidebar.multiselect("Criticidad", options=crit_available)
if selected_crit:
    df_filtered = df_filtered[df_filtered["CRITICIDAD"].isin(selected_crit)]

causes_available = sorted(df_filtered["CAUSA"].unique().tolist())
selected_causes = st.sidebar.multiselect("Causa del Evento", options=causes_available)
if selected_causes:
    df_filtered = df_filtered[df_filtered["CAUSA"].isin(selected_causes)]

techs_available = sorted(df_filtered["TECNOLOGIAS AFECTADAS"].unique().tolist())
selected_techs = st.sidebar.multiselect("Tecnología Afectada", options=techs_available)
if selected_techs:
    df_filtered = df_filtered[df_filtered["TECNOLOGIAS AFECTADAS"].isin(selected_techs)]
...
# =============================================================================
# FASE 3: PANEL DE ALERTAS Y SITIOS RECURRENTES (TOP 10)
# =============================================================================
...
# =============================================================================
# FASE 4: GRÁFICOS INTERACTIVOS DE ANÁLISIS (PLOTLY)
# =============================================================================
...
# =============================================================================
# FASE 5: TABLA EJECUTIVA DETALLADA Y EXPORTACIÓN DE DATOS
# =============================================================================
...
# Renderizar tabla interactiva de Streamlit
st.dataframe(
...
# Generador de Excel descargable en memoria (BytesIO)
